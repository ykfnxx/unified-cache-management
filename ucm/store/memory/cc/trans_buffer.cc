...
Status TransBuffer::Load(Detail::TaskDesc& task)
{
    ...
    for (const auto& shard : task) {
        ...
        if (s.Failure()) { return s; }
        s = CopyToAddrs(full.data(), tensorSizeList_, shard.addrs);
        if (s.Failure()) { return s; }
    }
    return Status::OK();
}
...
Status TransBuffer::Dump(const Detail::TaskDesc& task)
{
    for (const auto& shard : task) {
        ...
        auto s = CopyFromAddrs(shard.addrs, tensorSizeList_, full.data());
        if (s.Failure()) { return s; }
...
Status TransBuffer::CommitFull(const BlockId& block, size_t layer, const std::vector<std::byte>& full)
{
    std::lock_guard<std::mutex> guard(mutex_);
